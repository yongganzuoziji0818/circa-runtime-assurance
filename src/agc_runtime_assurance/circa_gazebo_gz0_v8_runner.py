"""Non-running scientific runner and lossless evidence codec for GZ0-v8.

The module contains the future exactly-once execution path, but the current
schema-only manifest cannot activate it: it has no scientific seed material,
output path, or launch authorization.  The only presently executable path is
the in-memory schema/capacity audit.
"""
...
class CircaGazeboGZ0V8RunnerError(RuntimeError):
...
SCHEMA_VERSION = "circa-gz0-v8-summary-json-lossless-arrays-v1"
SEED_NAMESPACE = "circa-gz0-v8"
...
def _safe_child(root: Path, relative: str) -> Path:
    ...
    if root not in path.parents:
        raise CircaGazeboGZ0V8RunnerError(f"unsafe path outside repository: {relative}")
    return path


def _binary_hash(value: str) -> np.ndarray:
    if len(value) != 64 or any(ch not in "0123456789abcdef" for ch in value):
        raise CircaGazeboGZ0V8RunnerError("invalid lowercase SHA-256 value")
...
def array_schema(rollouts: int, horizon: int) -> dict[str, tuple[np.dtype, tuple[int, ...]]]:
    if rollouts <= 0 or horizon <= 0:
        raise CircaGazeboGZ0V8RunnerError("rollouts and horizon must be positive")
    rh = (rollouts, horizon)
    return {
...
def validate_arrays(arrays: Mapping[str, np.ndarray], rollouts: int, horizon: int) -> None:
    ...
    if set(arrays) != set(expected):
        raise CircaGazeboGZ0V8RunnerError("lossless array member set drifted")
    for name, (dtype, shape) in expected.items():
        value = np.asarray(arrays[name])
        if value.dtype != dtype or value.shape != shape or value.dtype.hasobject:
            raise CircaGazeboGZ0V8RunnerError(f"array schema mismatch: {name}")
...
def verify_lossless_arrays(payload: bytes, expected: Mapping[str, np.ndarray]) -> dict[str, Any]:
    with np.load(BytesIO(payload), allow_pickle=False) as archive:
        if set(archive.files) != set(expected):
            raise CircaGazeboGZ0V8RunnerError("archive member set drifted")
        for name, original in expected.items():
            restored = archive[name]
            if restored.dtype != original.dtype or restored.shape != original.shape:
                raise CircaGazeboGZ0V8RunnerError(f"archive schema drifted: {name}")
            if restored.tobytes() != original.tobytes():
                raise CircaGazeboGZ0V8RunnerError(f"archive round trip is not lossless: {name}")
...
def schema_fixture_arrays(
    ...
    for offset, name in enumerate(
        ("source_center_radius_set", "aligned_center_radius_set", "selected_action")
    ):
    ...
    for offset, name in enumerate(
        ("applied_action_history_digest", "provenance_hash", "uncertainty_registry_hash")
    ):
...
def materialize_required_fields(arrays: Mapping[str, np.ndarray]) -> dict[str, np.ndarray]:
    ...
    if np.any(candidates >= 12):
        raise CircaGazeboGZ0V8RunnerError("candidate template index is invalid")
...
def schema_capacity_audit(
    *, rollouts: int, horizon: int, output_bytes_max: int
    ...
    if output_bytes_max <= 0:
        raise CircaGazeboGZ0V8RunnerError("output budget must be positive")
...
def derive_scenario_seed(master_seed: int, candidate_id: str, index: int) -> int:
    if not isinstance(master_seed, int) or master_seed < 0 or not candidate_id or index < 0:
        raise CircaGazeboGZ0V8RunnerError("invalid future scientific seed input")
    digest = hashlib.sha256(
...
def compile_schedule(manifest: Mapping[str, Any]) -> list[FactorialRun]:
    if "master_seed" not in manifest:
        raise CircaGazeboGZ0V8RunnerError("scientific seed material is absent")
    ...
    if len(schedule) != int(manifest["max_regime_rollouts"]):
        raise CircaGazeboGZ0V8RunnerError("factorial schedule size drifted")
...
def validate_schema_only_manifest(manifest: Mapping[str, Any], root: Path) -> dict[str, Any]:
    expected = {
        "status": "RUNNER_IMPLEMENTED_NONRUNNABLE_SCHEMA_AUDIT_ONLY",
        "local_runner_implementation_authorized": True,
        "schema_capacity_audit_authorized": True,
    ...
    for key, value in expected.items():
        if manifest.get(key) != value:
            raise CircaGazeboGZ0V8RunnerError(f"schema manifest {key} drifted")
    for forbidden in ("master_seed", "schedule_seed", "seed_namespace", "output_path"):
        if forbidden in manifest:
            raise CircaGazeboGZ0V8RunnerError(f"forbidden runtime field present: {forbidden}")
    base = _safe_child(root, manifest["design_source_manifest_path"])
    if _sha256(base) != manifest["design_source_manifest_sha256"]:
        raise CircaGazeboGZ0V8RunnerError("frozen design hash mismatch")
    design = json.loads(base.read_text(encoding="utf-8"))
    if tuple(design["drivers"]) != DRIVERS or tuple(design["regimes_per_driver"]) != METHODS:
        raise CircaGazeboGZ0V8RunnerError("frozen factorial levels drifted")
    if tuple(design["trace_required_fields"]) != TRACE_REQUIRED_FIELDS:
        raise CircaGazeboGZ0V8RunnerError("frozen trace fields drifted")
    expected_rollouts = (
    ...
    if expected_rollouts != 600 or expected_rollouts != int(design["max_regime_rollouts"]):
        raise CircaGazeboGZ0V8RunnerError("frozen factorial size drifted")
    if (root / design["output_namespace_reserved"]).exists():
        raise CircaGazeboGZ0V8RunnerError("reserved scientific output must remain absent")
    for relative, expected_hash in manifest.get("protected_files", {}).items():
        if _sha256(_safe_child(root, relative)) != expected_hash:
            raise CircaGazeboGZ0V8RunnerError(f"protected hash mismatch: {relative}")
    return design
...
def _validate_runnable_manifest(
    ...
    for key, expected in required.items():
        if manifest.get(key) != expected:
            raise CircaGazeboGZ0V8RunnerError(f"runnable manifest {key} drifted")
    ...
    if _sha256(design_path) != manifest["design_source_manifest_sha256"]:
        raise CircaGazeboGZ0V8RunnerError("runnable design lock failed")
    ...
    if drift:
        raise CircaGazeboGZ0V8RunnerError(f"runnable scientific payload drifted: {drift}")
    if tuple(manifest["drivers"]) != DRIVERS or tuple(manifest["regimes_per_driver"]) != METHODS:
        raise CircaGazeboGZ0V8RunnerError("runnable factorial levels drifted")
    if tuple(manifest["trace_required_fields"]) != TRACE_REQUIRED_FIELDS:
        raise CircaGazeboGZ0V8RunnerError("runnable trace fields drifted")
    for group in ("source_files", "protected_files"):
        if not manifest.get(group):
            raise CircaGazeboGZ0V8RunnerError(f"{group} lock is empty")
        for relative, expected_hash in manifest[group].items():
            if _sha256(_safe_child(root, relative)) != expected_hash:
                raise CircaGazeboGZ0V8RunnerError(f"{group} hash mismatch: {relative}")
    world = _safe_child(root, manifest["world_path"])
    if _sha256(world) != manifest["world_sha256"]:
        raise CircaGazeboGZ0V8RunnerError("world hash mismatch")
    output = _safe_child(root, manifest["output_path"])
    if output.exists():
        raise CircaGazeboGZ0V8RunnerError("exactly-once output already exists")
    ...
    if len(schedule) != 600 or len(seeds) != 60:
        raise CircaGazeboGZ0V8RunnerError("runnable paired schedule is invalid")
...
def _reason_index(reason: str) -> int:
    ...
    try:
        ...
        raise CircaGazeboGZ0V8RunnerError(f"unregistered intervention reason: {reason}") from error
...
def _environment(
    ...
    raise CircaGazeboGZ0V8RunnerError("unknown frozen driver")
...
def _run_one(
    ...
    for step in range(int(design["horizon_steps"])):
        ...
        if method == "shadow_no_override":
            ...
            reason = "robust_backup_filter_v7"
...
def run_gz0_v8(manifest_path: str | Path, repo_root: str | Path) -> dict[str, Any]:
    root = Path(repo_root).resolve()
    path = Path(manifest_path).resolve()
    if root not in path.parents or not path.is_file():
        raise CircaGazeboGZ0V8RunnerError("unsafe or absent runnable manifest")
    raw = path.read_bytes()
    ...
    for row, spec in enumerate(schedule):
        if time.perf_counter() - start > float(design["wall_time_seconds_max"]):
            raise CircaGazeboGZ0V8RunnerError("v8 wall-time budget exceeded")
    ...
    if total > int(design["output_bytes_max"]):
        raise CircaGazeboGZ0V8RunnerError("v8 lossless evidence exceeds output budget")
    output = _safe_child(root, manifest["output_path"])
    output.mkdir(parents=True, exist_ok=False)
    (output / "result.json").write_bytes(summary)
    (output / "trace_arrays.npz").write_bytes(archive)
    return {
        "status": "COMPLETE_CLAIM_INELIGIBLE_GZ0_V8",
...
def main() -> None:
    ...
    if args.schema_only == args.run:
        raise SystemExit("select exactly one of --schema-only or --run")
    if args.run:
        print(json.dumps(run_gz0_v8(args.manifest, args.repo_root), sort_keys=True))
        return
    ...
    result = schema_capacity_audit(
        rollouts=int(design["max_regime_rollouts"]),
        horizon=int(design["horizon_steps"]),
        output_bytes_max=int(design["output_bytes_max"]),
    )
