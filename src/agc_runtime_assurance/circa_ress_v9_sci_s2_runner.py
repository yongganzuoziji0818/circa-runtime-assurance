"""Bounded-memory execution successor for the frozen CIRCA-RESS-V9 design.
...
SUCCESSOR_ID = "CIRCA-RESS-V9-SCI-S2"
SEGMENT_ROLLOUTS = 64
...
def _worker(
    ...
    with _v9._configured_core():
        design, operationalization, registry, _output, _state = (
            _v9._validate_v9_manifest(manifest, repo_root)
        )
...
def run(manifest_path: str | Path, repo_root: str | Path) -> dict[str, Any]:
    ...
    expected_runner_hash = manifest.get("source_hashes", {}).get(
        "src/agc_runtime_assurance/circa_ress_v9_sci_s2_runner.py"
    )
    ...
    try:
        for start in range(0, _v9.ROLLOUTS, SEGMENT_ROLLOUTS):
            ...
            command = [
                sys.executable,
                "-m",
                "agc_runtime_assurance.circa_ress_v9_sci_s2_runner",
                "--worker",
                "--manifest",
                str(manifest_file),
                "--repo-root",
